...
evi(controllo_condizioni):-fase(inizializzazione),write('--------------------'),nl,random(1,5,var_R),retract(condizione(4)),assert(condizione(var_R)),retract(fase(inizializzazione)).

evi(controllo_condizioni):- \+fase(inizializzazione),(condizione(3)->ordine(medicina);condizione(2)->ordine(acqua);condizione(1)->ordine(cibo);write(''),aggiorna).

ordine(var_Y):-id_ordine(var_X),var_X==0,write('Salve, ho un ordine da inviare.'),nl,a(message(venditore,send_message(spesa([var_Y,var_Me]),var_Me))),stampa_richiesta(var_Y).

ordine(var_Y):-id_ordine(var_X),var_X>0,write('').

stampa_richiesta(var_Y):-write('Ho ordinato: '),write(var_Y),write('.').

aggiorna:-random(1,5,var_R),retract(condizione(4)),assert(condizione(var_R)).

eve(assegna_ordine_numero(var_Y)):-retract(id_ordine(var_X)),assert(id_ordine(var_Y)),write('L ID del mio ordine e: '),write(var_Y),write('.').

eve(ritorna_piu_tardi):-write('Ok').

eve(richiesta_id):-id_ordine(var_X),var_X==0,a(message(venditore,send_message(risposta_id([var_X,var_Me]),var_Me))),write('Non ho effettuato ordini.'),nl,write('--------------------').

eve(richiesta_id):-id_ordine(var_X),var_X>0,a(message(venditore,send_message(risposta_id([var_X,var_Me]),var_Me))).

eve(rimuovi_ordine(var_X)):-id_ordine(var_X),var_X==0,write('Non ho ordini, c e stato un errore.'),nl,write('--------------------'),nl.

eve(rimuovi_ordine(var_X)):-id_ordine(var_X),var_X>0,retract(id_ordine(var_X)),assert(id_ordine(0)),write('Grazie per il servizio, a presto.'),nl,write('--------------------'),retract(condizione(var_Z)),random(1,5,var_R),assert(condizione(var_R)),nl,var_R==4,write('Per adesso non ho bisogno di niente.'),nl,write('--------------------'),nl.

eve(ritenta_ordine):-write('Ok, ritento piu tardi'),nl,write('--------------------'),nl.
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_122917,var_Ontology,_122921),_122911),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_122955)),a(message(var_Ag,propose(var_A,[_122955],var_AgI))),retractall(ext_agent(var_Ag,_122993,var_Ontology,_122997)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_122791,var_Ontology,_122795),_122785),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_122861,var_Ontology,_122865)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_122679,var_Ontology,_122683),_122673),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_122735,var_Ontology,_122739)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_122243),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_122277),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_122059).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_121907),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_121855),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_121731,_121733)),clause(agente(_121753,_121755,_121757,var_S),_121749),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_121525,_121527,_121529,var_S),_121521),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_121625,[]),repeat,read(_121625,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_121625).
