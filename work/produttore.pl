...
evi(sveglia):-contatore_richiesta(var_Q0),contatore_servito(var_Q1),var_Q0==var_Q1,write('').

evi(sveglia):-contatore_richiesta(var_Q0),contatore_servito(var_Q1),var_Q0>var_Q1,servi_prodotto(var_Q1,var_Q2).

eve(richiesta_id):-write('').

eve(richiesta_merce(var_Z)):-write('--------------------'),nl,write('Ho ricevuto la richiesta di rifornimenti per il bene: '),write(var_Z),write('.'),nl,registra_richiesta(var_Z,var_Q0,var_Q1).

registra_richiesta(var_Z,var_Q0,var_Q1):-contatore_richiesta(var_Q0),var_Q1 is var_Q0+1,assert(richiesta(var_Z,var_Q1)),retract(contatore_richiesta(var_Q0)),assert(contatore_richiesta(var_Q1)).

servi_prodotto(var_Q1,var_Q2):-var_Q2 is var_Q1+1,richiesta(var_X,var_Q2),a(message(venditore,send_message(arrivo_merci(var_X),var_Me))),write('Invio rifornimenti di: '),write(var_X),write(' al venditore.'),retract(richiesta(var_X,var_Q2)),retract(contatore_servito(var_Q1)),assert(contatore_servito(var_Q2)).

servi_prodotto(var_Q1,var_Q2):-var_Q2 is var_Q1+1,\+richiesta(var_X,var_Q2),write('Qualcosa e andato storto con la registrazione delle richieste.'),nl,retract(richiesta(var_X,var_Q2)),retract(contatore_servito(var_Q1)),assert(contatore_servito(var_Q2)).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_100709,var_Ontology,_100713),_100703),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_100747)),a(message(var_Ag,propose(var_A,[_100747],var_AgI))),retractall(ext_agent(var_Ag,_100785,var_Ontology,_100789)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_100583,var_Ontology,_100587),_100577),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_100653,var_Ontology,_100657)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_100471,var_Ontology,_100475),_100465),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_100527,var_Ontology,_100531)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_100035),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_100069),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_99851).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_99699),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_99647),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_99523,_99525)),clause(agente(_99545,_99547,_99549,var_S),_99541),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_99317,_99319,_99321,var_S),_99313),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_99417,[]),repeat,read(_99417,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_99417).
