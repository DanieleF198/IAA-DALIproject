...
evi(sveglia):-write('--------------------'),nl,write('Eccomi! Inizio il turno.'),nl,inizio_routine,write('Controllo ordini...'),nl.

eve(spesa(var_Z)):-var_Z=[var_Bene,var_Mittente],write('Ho ricevuto delle richieste mentre dormivo.'),aggiorna_numero_ordine(var_X,var_Y),ordine_numero(var_W),assert(bene_da_servire(var_W,var_Bene)),a(message(var_Mittente,send_message(assegna_ordine_numero(var_W),var_Me))).

aggiorna_numero_ordine(var_X,var_Y):-ordine_numero(var_X),var_Y is var_X+1,retract(ordine_numero(var_X)),assert(ordine_numero(var_Y)).

inizio_routine:-a(message(all,send_message(richiesta_id,var_Me))).

eve(risposta_id(var_X)):- \+ordine(var_A,var_Mittente),var_X=[var_Y,var_Z],aggiorna_contatore(var_Q0,var_Q1),(var_Y>0->assert(ordine(var_Y,var_Z));write('')),(var_Q1==2->assert(pronto(servizio));write('')).

eve(risposta_id(var_X)):-ordine(var_A,var_Mittente),var_X=[var_Y,var_Z],aggiorna_contatore(var_Q0,var_Q1),(var_Y>var_A->write('');var_Y>0->retract(ordine(var_A,var_Mittente)),assert(ordine(var_Y,var_Z));write('')),(var_Q1==2->assert(pronto(servizio));write('')).

aggiorna_contatore(var_Q0,var_Q1):-contatore_risposte(var_Q0),var_Q1 is var_Q0+1,retract(contatore_risposte(var_Q0)),assert(contatore_risposte(var_Q1)).

servizio_routine:-pronto(servizio).

evi(servizio_routine):- \+ordine(var_X,var_Y),write('Non ho ricevuto ordini, chiudo il turno.'),nl,write('--------------------'),nl,retract(contatore_risposte(2)),assert(contatore_risposte(0)),retract(pronto(servizio)).

evi(servizio_routine):-ordine(var_X,var_Y),bene_da_servire(var_X,var_Z),bene_in_magazzino(var_Z,var_W),(var_W>0->write('Servo l ordine di: '),write(var_Y),write(' con id: '),write(var_X),write('.'),nl,write('Fatto!'),nl,write('Chiudo il turno.'),nl,write('--------------------'),aggiorna_bene_in_magazzino(var_Z,var_Q0,var_Q1);write('Bene momentaneamente non disponibile, provi a ripassare presto. Chiudo il turno.'),nl,write('--------------------'),nl),retract(contatore_risposte(2)),assert(contatore_risposte(0)),retract(ordine(var_X,var_Y)),rispondi_consumatore(var_W,var_X,var_Y),retract(pronto(servizio)).

rispondi_consumatore(var_W,var_X,var_Y):-var_W>0,a(message(var_Y,send_message(rimuovi_ordine(var_X),var_Me))).

rispondi_consumatore(var_W,var_X,var_Y):-var_W==0,a(message(var_Y,send_message(ritenta_ordine,var_Me))).

aggiorna_bene_in_magazzino(var_Z,var_Q0,var_Q1):-bene_in_magazzino(var_Z,var_Q0),var_Q1 is var_Q0-1,(var_Q1==0->nl,write('In magazzino non ci sono piu scorte di '),write(var_Z),write(', provvedo a richiedere rifornimenti al produttore.'),nl,write('--------------------'),acquista_merci_routine(var_Z);retract(bene_in_magazzino(var_Z,var_Q0)),assert(bene_in_magazzino(var_Z,var_Q1))).

acquista_merci_routine(var_Z):-a(message(produttore,send_message(richiesta_merce(var_Z),var_Me))).

eve(arrivo_merci(var_X)):-write('***********************************'),nl,write('Sono arrivati rifornimenti di '),write(var_X),write('.'),nl,write('***********************************'),nl,retract(bene_in_magazzino(var_X,var_Y)),assert(bene_in_magazzino(var_X,3)).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_158875,var_Ontology,_158879),_158869),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_158913)),a(message(var_Ag,propose(var_A,[_158913],var_AgI))),retractall(ext_agent(var_Ag,_158951,var_Ontology,_158955)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_158749,var_Ontology,_158753),_158743),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_158819,var_Ontology,_158823)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_158637,var_Ontology,_158641),_158631),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_158693,var_Ontology,_158697)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_158201),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_158235),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_158017).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_157865),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_157813),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_157689,_157691)),clause(agente(_157711,_157713,_157715,var_S),_157707),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_157483,_157485,_157487,var_S),_157479),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_157583,[]),repeat,read(_157583,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_157583).
